...
public class WebSocketServer {
     ...
     */
    @OnClose
    public void onClose(@PathParam("room") String room, @PathParam("username") String username) {
        Map<User, Session> sessionMap = rooms.get(room);
...
//    -----------------------------------------------
     ...
     * @param room 房间号
     */
    private void sendMessageToUser(String message, String username, String room) {
        try {
            Map<User, Session> sessionMap = rooms.get(room);
            if (sessionMap != null) {
                for (User user : sessionMap.keySet()) {
        ...
        if (sessionMap != null) {
            for (User user : sessionMap.keySet()) {
                if (user.getUsername().equals(newUser)) {
                    sendMessageToUser(message, user.getUsername(), room);
                    break; // 只发送给新用户一次
                }
            }
        }
